...
export default class ComtCache {
    ...
    loadedFragments = [];
    clearLoadedFragments(){
        this.loadedFragments.length = 0;
    }
    ...
    private async fetchIndexEntry(tmsIndex: TmsIndex, cancellationToken: CancellationToken): Promise<IndexEntry> {
        ...
        if (!this.requestCache.has(fragmentRange.startOffset)) {
			...
			this.loadedFragments.push({tmsIndex, range: fragmentRange.index});
			console.info("requested index fragment: ", startOffset , endOffset, fragmentRange.index, tmsIndex);
