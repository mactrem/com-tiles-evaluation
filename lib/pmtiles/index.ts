...
async function fflateDecompress(buf: ArrayBuffer, compression: Compression): Promise<ArrayBuffer> {
...
export const directoryInfo: any[] = [];
export  function clearDirectoryInfo(){
...
async function getDirectory(
    ...
    //console.time("decompress directory");
    const data = await decompress(resp.data, header.internalCompression);
    const directory = deserializeIndex(data);
    //console.timeEnd("decompress directory");
    directoryInfo.push({size: resp.data.byteLength, length: directory.length, lengthTotal: directory.reduce((p, c) => p + c.runLength, 0)});
...
export class ResolvedValueCache {
    ...
    constructor(maxCacheEntries = 100, prefetch = true, decompress: DecompressFunc = fflateDecompress) {
    ...
    async getHeader(source: Source, current_etag?: string): Promise<Header> {
        ...
        const res = await getHeaderAndRoot(source, this.decompress, this.prefetch, current_etag);
        if (res[1]) {
...
export class SharedPromiseCache {
    ...
    prune() {
        if (this.cache.size >= this.maxCacheEntries) {
            let minUsed = Infinity;
            let minKey = undefined;
            this.cache.forEach((cache_value: SharedPromiseCacheValue, key: string) => {
                if (cache_value.lastUsed < minUsed) {
                    minUsed = cache_value.lastUsed;
                    minKey = key;
                }
...
export class PMTiles {
    ...
    async getZxyAttempt(z: number, x: number, y: number, signal?: AbortSignal): Promise<RangeResponse | undefined> {
        ...
        for (let depth = 0; depth <= 3; depth++) {
            const directory = await this.cache.getDirectory(this.source, d_o, d_l, header);
    ...
    async getMetadataAttempt(): Promise<any> {
        ...
        const resp = await this.source.getBytes(header.jsonMetadataOffset, header.jsonMetadataLength);
        if (header.etag && header.etag !== resp.etag) {
            throw new EtagMismatch(resp.etag);
        }
...
export async function decodeDirectory(compressedDirectory: ArrayBuffer, compression: Compression = Compression.Gzip) {
    const data = await fflateDecompress(compressedDirectory, compression);
    return deserializeIndex(data);
}

function fflateDecompressSync(buf: ArrayBuffer, compression: Compression): ArrayBuffer {
    if (compression === Compression.None || compression === Compression.Unknown) {
        return buf;
    } else if (compression === Compression.Gzip) {
        return decompressSync(new Uint8Array(buf));
...
export function decodeDirectorySync(compressedDirectory: ArrayBuffer, compression: Compression = Compression.Gzip) {
    const data = fflateDecompressSync(compressedDirectory, compression);
    return deserializeIndex(data);
}
